...
//함수로 빼보고 유아트 새로운 파일로 분리시키고 메인과 유아트가 같이 쓰는 변수는 넘겨주고 메인 엑스턴 헤더 인클루드 정의전에 def꼭 해주고 

extern volatile unsigned char rx2Flag; //extern?
extern char rx2Data[50]; //전역변수는 두번선언안됨
...
void Serial2_Event()//메인함수에서 기능을 추가해야하는거는 빼면안됨
{
  int i=0;
  int num = 0;
  char * pToken;
