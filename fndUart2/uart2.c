#include "stm32f4xx.h"//기본적으로 필요한 파일
#include "uart2.h"


volatile unsigned char rx2Flag = 0;//인터럽트 서비스루틴과 메인 스페이스 공유를 위한
char rx2Data[50]; //버퍼
...
void Serial2_Send(unsigned char t)
{
  ...
  while (USART_GetFlagStatus(USART2, USART_FLAG_TXE) == RESET);//보내고 있는 중에 오버라이트될수도 보내는 동안 대기
}
...
void Serial2_Send_String(char* s)
{
  while( *s != '\0')
  {
   Serial2_Send((unsigned char)(*s));//그다음문자를 가르키는 주소로
   s++;   //s = s + 1;
  }
}
